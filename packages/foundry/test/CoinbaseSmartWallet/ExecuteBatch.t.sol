...
contract TestExecuteWithoutChainIdValidation is SmartWalletTestBase, TestPlus {
    function testExecuteBatch() public {
        ...
        TimelockSmartWallet.Call[] memory calls = new TimelockSmartWallet.Call[](2);
    ...
    function testExecuteBatch(uint256 r) public {
        ...
        unchecked {
            uint256 n = r & 3;
            TimelockSmartWallet.Call[] memory calls = new TimelockSmartWallet.Call[](n);
            ...
            if (_random() & 1 == 0) {
                MockTimelockSmartWallet(payable(address(account))).executeBatch(_random(), calls);
            } else {
                account.executeBatch(calls);
            }
