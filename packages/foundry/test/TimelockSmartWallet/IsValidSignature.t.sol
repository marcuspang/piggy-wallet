...
contract TestIsValidSignature is TimelockSmartWalletTestBase {
    ...
    function testSmartWalletSigner() public {
        ...
        TimelockSmartWallet.SignatureWrapper memory wrapperForOtherAccount =
            TimelockSmartWallet.SignatureWrapper(0, signature);

        bytes memory sig = abi.encode(
            TimelockSmartWallet.SignatureWrapper({ownerIndex: 2, signatureData: abi.encode(wrapperForOtherAccount)})
        );
    ...
    function testValidateSignatureWithEOASigner() public {
        ...
        bytes memory signature = abi.encodePacked(r, s, v);
        bytes4 ret = account.isValidSignature(hash, abi.encode(TimelockSmartWallet.SignatureWrapper(0, signature)));
        assertEq(ret, bytes4(0x1626ba7e));
    }
    ...
    function testRevertsIfEthereumSignatureButWrongOwnerLength() public {
        ...
        bytes memory signature = abi.encodePacked(r, s, v);
        vm.expectRevert();
        account.isValidSignature(hash, abi.encode(TimelockSmartWallet.SignatureWrapper(1, signature)));
    }
    ...
    function testRevertsIfOwnerIsInvalidEthereumAddress() public {
        ...
        account.isValidSignature(hash, abi.encode(TimelockSmartWallet.SignatureWrapper(0, signature)));
    }
}
