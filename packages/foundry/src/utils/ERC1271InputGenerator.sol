...
///         TimelockSmartWallet. May be useful for generating ERC-6492 compliant signatures.
...
contract ERC1271InputGenerator {
    ...
    /// @notice Computes and returns the expected ERC-1271 replay-safe hash for a TimelockSmartWallet.
    ///
    ...
    constructor(TimelockSmartWallet account, bytes32 hash, address accountFactory, bytes memory factoryCalldata) {
        ...
        bytes32 replaySafeHash = _smartWallet1271Input(account, hash, accountFactory, factoryCalldata);
        assembly {
    ...
    function _smartWallet1271Input(
