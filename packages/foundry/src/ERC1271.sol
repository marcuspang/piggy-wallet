...
abstract contract ERC1271 {
    ...
    bytes32 private constant _MESSAGE_TYPEHASH = keccak256("TimelockSmartWalletMessage(bytes32 hash)");
    ...
    ///         hashStruct(TimelockSmartWalletMessage({ hash: `hash`}))
    ///      )
    ///
    /// @param hash The original hash.
    ///
    ...
    /// @notice Returns the EIP-712 typed hash of the `TimelockSmartWalletMessage(bytes32 hash)` data structure.
    ///
    ...
    ///
    /// @param hash The `TimelockSmartWalletMessage.hash` field to hash.
    ////
    ...
    /// @notice Returns the EIP-712 `hashStruct` result of the `TimelockSmartWalletMessage(bytes32 hash)` data
    ///         structure.
    ///
    ...
    ///
    /// @param hash The `TimelockSmartWalletMessage.hash` field.
    ///
